...
const renderTextReport = (
	...
	if (context.transform.metrics.buildEvents.length) {
		const longest = Number(
			context.transform.metrics.summary?.longestBuildMs ?? 0,
		);
		lines.push("Build Metrics:");
		lines.push(
			`${indent}Events: ${context.transform.metrics.buildEvents.length}`,
		);
		if (longest) {
	...
	if (result.issues.length) {
		...
		const limit = options.verbose
			...
			: Math.min(10, result.issues.length);
...
export const writeTextReport = async (
	...
	const outputPath =
		options.outputPath ?? path.join(DEFAULT_REPORT_DIR, "report.txt");
...
export const writeMarkdownReport = async (
	...
	const cwd = options.cwd ?? result.context.cwd;
	const outputPath =
		options.outputPath ?? path.join(DEFAULT_REPORT_DIR, "report.md");
	const absolutePath = path.resolve(cwd, outputPath);
...
const renderMarkdownReport = (
	...
	if (context.transform.metrics.buildEvents.length) {
		...
		lines.push("");
		lines.push(
			`- Total build events: ${context.transform.metrics.buildEvents.length}`,
		);
		const longest = Number(
			context.transform.metrics.summary?.longestBuildMs ?? 0,
		);
...
export const writeJsonReport = async (
	...
	const outputPath =
		options.outputPath ??
		path.join(DEFAULT_REPORT_DIR, DEFAULT_REPORT_FILENAME);
	const absolutePath = path.resolve(cwd, outputPath);
	...
	await fs.writeFile(
		absolutePath,
		JSON.stringify(payload, undefined, indent),
		"utf8",
	);
...
const resolveIndent = (
	pretty: JsonReportOptions["pretty"],
): number | undefined => {
	if (pretty === false) {
		return undefined;
	}
